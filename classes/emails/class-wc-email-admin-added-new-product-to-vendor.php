...
class WC_Email_Admin_Added_New_Product_to_Vendor extends WC_Email {
	...
	function __construct() {
		...
		$this->title       = __( 'New Vendor Product By Admin', $DC_Product_Vendor->text_domain );
		$this->description = __( 'New order emails are sent when a new product is submitted to vendor by a Admin', $DC_Product_Vendor->text_domain );

		$this->heading = __( 'New product submitted: {product_name}', $DC_Product_Vendor->text_domain );
		$this->subject = __( '[{blogname}] Admin submitted new product to You - {product_name}', $DC_Product_Vendor->text_domain );
	...
	function trigger( $post_id, $post, $vendor )	{
		...
		if($DC_Product_Vendor->vendor_caps->vendor_capabilities_settings('is_submit_product') && get_user_meta($vendor->id, '_vendor_submit_product' ,true)) { 
